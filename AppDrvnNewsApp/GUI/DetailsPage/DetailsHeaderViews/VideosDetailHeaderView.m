...
@interface VideosDetailHeaderView()<LCBannerViewDelegate, UIWebViewDelegate>
...
@implementation VideosDetailHeaderView
...
- (void) updateDisplay:(NewsModel *)model
{
    ...
    NSString *htmlString = [NSString stringWithFormat:@"<html><body text=\"#FFFFFF\">%@</body></html>", model.htmlContent];
...
{
    if ([self.delegate respondsToSelector:@selector(VideosDetailHeaderViewDelegateDidSelectedAtIndex:)])
    {
        [self.delegate VideosDetailHeaderViewDelegateDidSelectedAtIndex:index];
    }
}
...
- (void)webViewDidFinishLoad:(UIWebView *)webView
{
    ...
    if ([self.delegate respondsToSelector:@selector(VideosDetailHeaderViewDelegateUpdateHeight:)])
    {
        [self.delegate VideosDetailHeaderViewDelegateUpdateHeight:[result floatValue] + 57 + self.titleLabel.frame.size.height + BANNER_VIEW_HEIGHT];
    }
}
